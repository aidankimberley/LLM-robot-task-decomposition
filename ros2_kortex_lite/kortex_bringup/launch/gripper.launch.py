...
def launch_setup(context, *args, **kwargs):
    ...
    joint_position_controller = LaunchConfiguration("joint_position_controller")
 ...
 #   world_file_path = [FindPackageShare('kortex_description'), '/worlds/final.sdf']
  ...
  #  print(f"DEBUG: Loading world from: {world_file_path}")
    gz_launch_description = IncludeLaunchDescription(
        ...
        launch_arguments={
            "gz_args": " -r -v 3 pick-and-place.sdf --physics-engine gz-physics-bullet-featherstone-plugin"
        }.items(),
        condition=IfCondition(sim_gazebo),
    )
...
def generate_launch_description():
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            "controllers_file",
            default_value="ros2_controllers_with_gripper.yaml",
